...
def openCSVFile(fileName):
    ...
    with open(fileName, newline='') as csvfile:
        ...
        for rowRemove in reversed(skipRows):
            ...
            goodRows.discard(rowRemove)

    return listOfData, rowNum, colNum, goodRows

listOfData, totalNumberOfVals, numberOfCols, goodRows = openCSVFile('dpData.csv')
...
def findBestK(whatPoint):
    ...
    centers = createListOfCenters(kClusters+1)
    lastBestSSE = sys.float_info.max
    numberOfIterations = 0
...
def graphClusters(whatPoint):
    ...
    for point in whatPoint.keys():
        ...
        elif(whatPoint[point] == 4):
            x4.append(listOfData[point][0])
            y4.append(listOfData[point][1])
            z4.append(listOfData[point][2])
        elif(whatPoint[point] == 5):
            x5.append(listOfData[point][0])
            y5.append(listOfData[point][1])
            z5.append(listOfData[point][2])
    ...
    ax.scatter(x4,y4,z4,c='purple')
    ax.scatter(x5,y5,z5,c='orange')

    ax.set_xlabel('Max Temp')
    ax.set_ylabel('Min Temp')
    ax.set_zlabel('Precip')
    ax.set_zlim(0,4)
...
def printKMeansCluster():
    ...
    for point in bestWhatPoint.keys():
        print(str(goodRows.pop()) + " center: " + str(bestWhatPoint.get(point)))
